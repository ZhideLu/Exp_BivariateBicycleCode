...
def get_SM_circuit(error_rate_sq_com, error_rate_cz_com, error_rate_idle_com, error_crosstalk, remove_X_list, remove_Z_list, lin_order, data_qubits, Xchecks, Zchecks, nbs, sX, sZ):
    ...
    circuit.append("H", Xcheck + [data_L[i] for i in [0,1,3,4,6,7]] + Zcheck)
    circuit.append("X", Xcheck + data_L + data_R + Zcheck)
...
##----------------------------------------------------------------------------------------------------------------------
    ...
    circuit.append("Y", [data_R[i] for i in [3,4,5,6,7,8]] + Zcheck)   
    circuit.append("X", Xcheck + data_L + [data_R[i] for i in [0,1,2]] )   
...
##-------------------------------------------------------------------------------------------------------------
    ...
    circuit.append("H", [data_L[i] for i in [0,1,3,4,6,7]]  + data_R )  #
    circuit.append("X", Xcheck + data_L + data_R + Zcheck)  #
    circuit.append("DEPOLARIZE1", Xcheck + data_L + data_R + Zcheck, error_rate_sq_com)
##-------------------------------------------------------------------------------------------------------------
    t = 3
    ...
    circuit.append("Y", Xcheck + [data_L[i] for i in [0,1,2,6,7,8]] + [data_R[i] for i in [0,1,3,4,6,7]] + Zcheck)  #
    circuit.append("X", [data_L[i] for i in [3,4,5]] + [data_R[i] for i in [2,5,8]] )  #
    circuit.append("DEPOLARIZE1", Xcheck + data_L + data_R + Zcheck, error_rate_sq_com)
##-------------------------------------------------------------------------------------------------------------
    t = 4
    data_qubits_cnoted_in_this_round = []
...
##-------------------------------------------------------------------------------------------------------------
    ...
    circuit.append("Y", Xcheck + [data_L[i] for i in [0,1,2,3,4,5] ] + [data_R[i] for i in [0,1,3,4,6,7] ] + Zcheck)  #
    circuit.append("X", [data_L[i] for i in [6,7,8] ] + [data_R[i] for i in [2,5,8] ] )  #
    circuit.append("H", data_L + data_R )  
    circuit.append("DEPOLARIZE1", Xcheck + data_L + data_R + Zcheck, error_rate_sq_com )
##--------------------------------------------------------------------------------------------------
    t = 6
    data_qubits_cnoted_in_this_round = []
...
##-------------------------------------------------------------------------------------------------------------
    ...
    circuit.append("X", data_R + Zcheck + [data_L[i] for i in [1,4,7]] )
    circuit.append("Y", Xcheck + [data_L[i] for i in [0,2,3,5,6,8]] )
